	...
	public function saveAction()
		{
			try {
				...
				throw new Exception("Invalid Request", 1);
					...
					$data = $payment->save();
					$message = new Model_Core_Message();
					$message->addMessages("Payment saved successfully.",Model_Core_Message::SUCCESS);
				$this->redirect("index.php?a=grid&c=payment");
				
			} catch (Exception $e) {
				$message = new Model_Core_Message();
				$message->addMessages("payment not saved.",Model_Core_Message::FAILURE);
				$this->redirect("index.php?a=grid&c=payment");
			}
		}
